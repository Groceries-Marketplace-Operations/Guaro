...
export default function Dashboard() {
  ...
  const canSeeBrands = hasPermission(account, 'brands.view');
  const canSeeTasks = hasPermission(account, 'tasks.view');

  const brandParams = isBpo ? { limit: 1, myBrands: true } : { limit: 1 };

  const { data: totalBrands }     = useQuery({ queryKey: ['brands', brandParams],                         queryFn: () => brandsApi.list(brandParams).then(r => (r.data as Paginated<unknown>).total), enabled: canSeeBrands });
  const { data: totalShops }      = useQuery({ queryKey: ['shops',  { limit: 1 }],                        queryFn: () => shopsApi.list({ limit: 1 }).then(r => (r.data as Paginated<unknown>).total), enabled: canSeeBrands });
  const { data: totalInProgress } = useQuery({ queryKey: ['tasks',  { limit: 1, status: 'in_progress' }], queryFn: () => tasksApi.list({ limit: 1, status: 'in_progress' }).then(r => (r.data as Paginated<unknown>).total), enabled: canSeeTasks });
  const { data: totalFailed }     = useQuery({ queryKey: ['tasks',  { limit: 1, status: 'failed' }],      queryFn: () => tasksApi.list({ limit: 1, status: 'failed' }).then(r => (r.data as Paginated<unknown>).total), enabled: canSeeTasks });
  const { data: totalDone }       = useQuery({ queryKey: ['tasks',  { limit: 1, status: 'done' }],        queryFn: () => tasksApi.list({ limit: 1, status: 'done' }).then(r => (r.data as Paginated<unknown>).total), enabled: canSeeTasks });
  const { data: recentResult }    = useQuery({ queryKey: ['tasks',  { limit: 8 }],                        queryFn: () => tasksApi.list({ limit: 8 }).then(r => r.data as Paginated<Task>), enabled: canSeeTasks });
    ...
    <>
      ...
      <main className="main-content">
        ...
        <div className="stat-grid">
          {canSeeBrands && <div className="stat-card orange-accent">
          ...
          {canSeeBrands && <div className="stat-card">
          ...
          {canSeeTasks && <div className="stat-card">
        ...
        {canSeeTasks && <div className="card">
